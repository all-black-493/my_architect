...
const AI_USER_ID = "my_architect-ai";
const AI_USER_INFO = { name: "My Architect AI", avatar: "", color: "#6457f9" };
...
function buildSystemPrompt(): string {
    ...
    return `You are My Architect AI, an expert system architect that generates technical architecture diagrams on a collaborative canvas.
    ...
    run: async (payload: { prompt: string; roomId: string; userId: string }) => {
        ...
        await lb
            .broadcastEvent(payload.roomId, {
                type: "ai-status",
                message: "My Architect AI is analyzing your request…",
                status: "start",
            })
            .catch(() => { });
        ...
        try {
            ...
            await lb
                .broadcastEvent(payload.roomId, {
                    type: "ai-status",
                    message: "My Architect AI encountered an error. Please try again.",
                    status: "error",
                })
                .catch(() => { });
            throw error;
